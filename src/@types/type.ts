...
export type Push<T extends any[], E> = ((head: E, ...args: T) => any) extends (head: infer Element, ...args: infer Array) => any
  ? [...Array, Element]
  : T;
...
export type _Recurse<T> = T extends { __rec: never }
  ? never
...
export type _Split<S extends string, D extends string, A extends any[] = []> =
  ...
  S extends `${infer T}${D}${infer U}` ? { __rec: _Split<U, D, Push<A, T>> } : A;
export type Split<S extends string, D extends string> = Recurse<_Split<S, D>>;

export type _CompareStringEqual<
  T extends string,
  U extends string,
  SplitedT extends any[] = Split<T, ''>,
  SplitedU extends any[] = Split<U, ''>,
  Temp extends any[] = [],
  Limit extends any[] = Push<SplitedU, 0>
> = SplitedU extends Temp
  ? true
  : Temp['length'] extends Limit['length']
  ? false
...
export type GetRouteInConfigId<T extends string, Pattern extends any[] = UnionToTuple<keyof LocationStates>> = Head<Pattern> extends
  | `/${any}/${any}`
  | `/${any}/:${any}` // Check page có id
  ? CompareStringEqual<T, GetTruePath<Head<Pattern>>> extends true
    ? Head<Pattern>
    : HasTail<Pattern> extends true
    ? GetRouteInConfigId<T, Tail<Pattern>>
    : undefined
  : Head<Pattern> extends `/`
  ? GetRouteInConfigId<T, Tail<Pattern>> // bỏ qua Page '/'
  : GetRouteInConfigId<T, Tail<Pattern>>;

export type GetRouteInConfigMultiple<
  T extends string,
  Pattern extends any[] = UnionToTuple<keyof LocationStates>
...
export type GetRouteInConfigOne<T extends string, Pattern extends any[] = UnionToTuple<keyof LocationStates>> = T extends Head<Pattern>
  ? Head<Pattern>
  : T extends `${Head<Pattern>}${'?' | '#'}${any}`
  ? Head<Pattern> extends '/'
    ? HasTail<Pattern> extends true
      ? GetRouteInConfigOne<T, Tail<Pattern>>
      : undefined
    : Head<Pattern>
  : HasTail<Pattern> extends true
  ? GetRouteInConfigOne<T, Tail<Pattern>>
...
export type GetState<T extends string> = GetRouteInConfig<T> extends keyof LocationStates
  ? LocationStates[GetRouteInConfig<T>]
